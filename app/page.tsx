...
export default function Page() {
  ...
  async function send() {
    ...
    const res = await fetch('/api/chat', {
      method: 'POST',
      body: JSON.stringify({ messages: next, companyId, companyName }),
    });
    ...
    <main>
      <div style={{display:'grid', gap:12}}>
        ...
        <div style={{display:'flex', gap:8}}>
          ...
          <button onClick={send} style={{padding:'12px 16px', borderRadius:10, background:'#12b886', color:'#04210f', border:'none', fontWeight:700}}>إرسال</button>
        </div>
        <p style={{opacity:.7, fontSize:12}}>ملاحظة: عدّاد الرسائل يتطلّب ضبط <code>DATABASE_URL</code>.</p>
      </div>
    </main>
  );
}
