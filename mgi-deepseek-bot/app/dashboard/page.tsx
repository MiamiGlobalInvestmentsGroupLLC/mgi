...
export default function Dashboard() {
  ...
  useEffect(() => {
    fetch('/api/usage').then(r=>r.json()).then(data=>{
      if (data.error) {
        // Show friendly message when DB not configured
        setCompanies([]);
        setUsages([]);
      } else {
  ...
  const totals = companies.map(c => {
    const sum = usages.filter(u=>u.companyId===c.id).reduce((a,b)=>a + (b.count||0), 0);
    return { company: c, total: sum };
    ...
    <main>
      ...
      {!companies.length ? (
        <div style={{padding:12, background:'#271b00', border:'1px solid #3a2a00', borderRadius:8}}>
          لم يتم إعداد قاعدة البيانات بعد. أضف متغيّر <code>DATABASE_URL</code> في Vercel لنشر العدّادات.
        </div>
      ) : (
